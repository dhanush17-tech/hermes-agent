...
export function parseIncomingMessage(text) {
    ...
    for (const { re, command } of COMMAND_PATTERNS) {
        ...
        if (m) {
            ...
            if (command === "research" && m[1])
                result.topic = m[1].trim();
            return result;
        }
    }
    return result;
}
export function formatParsedForOrchestrator(parsed) {
    if (parsed.command === "research" && parsed.topic)
        return `research ${parsed.topic}`;
    ...
    if (parsed.command === "deny" && parsed.approvalId)
        return `deny ${parsed.approvalId}`;
    if (parsed.command === "edit" && parsed.approvalId) {
        return `edit ${parsed.approvalId}: ${parsed.editInstruction ?? ""}`;
    }
    if (parsed.command === "show_approvals")
        return "show approvals";
    if (parsed.command === "risks_today")
        return "what could go wrong today";
    if (parsed.command === "morning_brief")
        return "daily brief";
    if (parsed.command === "evening_review")
        return "evening review";
    if (parsed.command === "emergency_stop")
        return "emergency stop";
    return parsed.raw;
}
//# sourceMappingURL=incoming-parser.js.map