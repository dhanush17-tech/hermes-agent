...
export function isAutomatedInbound(text: string, handle: string): boolean {
  ...
  if (norm.includes("(smsft)")) return true;

  for (const re of SERVICE_HANDLE) {
    if (re.test(norm) && !PHONE_OR_EMAIL.test(norm)) return true;
  }
...
export function shouldIgnoreInbound(text: string, handle: string): boolean {
