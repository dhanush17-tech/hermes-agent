...
export const DIGITAL_PRESENCE_SERVICES: PresenceService[] = [
...
export class DigitalPresenceMonitor {
  ...
  async scanNext(ctx: ToolContext): Promise<PresenceScanResult> {
    const service = DIGITAL_PRESENCE_SERVICES[
      this.rotateIndex % DIGITAL_PRESENCE_SERVICES.length
    ] as PresenceService;
    this.rotateIndex += 1;
    return this.scanService(service, ctx);
  }

  async scanAll(ctx: ToolContext): Promise<PresenceScanResult[]> {
    const results: PresenceScanResult[] = [];
    for (const svc of DIGITAL_PRESENCE_SERVICES) {
      results.push(await this.scanService(svc, ctx));
      if (process.env.VITEST !== "true") await delay(1500);
    }
    return results;
  }

  async scanService(service: PresenceService, ctx: ToolContext): Promise<PresenceScanResult> {
    ...
    try {
      ...
      const nav = await this.executor.invoke(
        "browser.open",
        { url: service.url },
        ctx,
        { summary: `Presence scan: ${service.label}` },
      );
      ...
      const pageId = (nav.data as { pageId?: string })?.pageId;
      await delay(NAV_MS);

      const obs = await this.executor.invoke(
        "browser.observe",
        { pageId },
        ctx,
        { summary: `DOM observe ${service.label}` },
      );

      if (obs.status === "success") {
        const observation = (obs.data as {
          observation?: { title?: string; url?: string; visibleText?: string; interactive?: Array<{ name: string; risk: string }> };
        })?.observation;
      ...
      const capturePath = (scr.data as { capturePath?: string })?.capturePath;
  ...
  private async scanGmailViaApi(_ctx: ToolContext): Promise<PresenceScanResult | null> {
    const multi = createMultiAccountGmailFromEnv();
    const accounts = loadGoogleAccountsFromEnv();
    if (!multi || !accounts.length) return null;
  ...
  private inferOpenLoopsFromText(text: string): string[] {
    return text
      .split("\n")
