...
export class MemoryAgent {
  ...
  async handle(userMessage: string, entities?: IntentEntities): Promise<string> {
    ...
    if (action === "forget") {
      ...
      if (!id) {
        ...
        if (!recent[0]) return "No memories to forget.";
  ...
  private async inferMemoryType(content: string): Promise<string> {
    if (!this.cf) return "durable_facts";
    const raw = await this.cf.chat(
      `Classify memory type for: ${content}`,
      {
        maxTokens: 80,
    ...
    try {
      ...
      const data = JSON.parse(raw.slice(start, end + 1)) as { memoryType?: string };
