...
const STEER_PHRASES =
  /\b(instead|rather|actually|wait|hold on|stop|don't|do not|not that|change|skip|focus on|try|use the|open the|go to|check the|before you|after you|yes|no|cancel that)\b/i;
 ...
 */
export async function classifySteeringRelevance(
  activeGoal: string,
  newMessage: string,
  ...
  if (!msg || !goal) return false;

  if (STEER_PHRASES.test(msg)) return true;

  const goalTokens = tokenize(goal);
  const msgTokens = tokenize(msg);
  const overlap = [...msgTokens].filter((t) => goalTokens.has(t)).length;
  if (overlap >= 2) return true;
  if (overlap >= 1 && msg.split(/\s+/).length <= 12) return true;
  ...
  try {
    ...
    if (start >= 0 && end > start) {
      const data = JSON.parse(raw.slice(start, end + 1)) as { related?: boolean };
