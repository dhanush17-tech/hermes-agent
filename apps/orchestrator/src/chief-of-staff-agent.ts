...
export class ChiefOfStaffAgent {
  ...
  constructor(
    ...
    private readonly hub: ConnectorHub | null,
    private readonly cf: CloudflareWorkersAIClient | null,
    private readonly contextGraph: ContextGraphService | null = null,
  ) {}

  async syncContextFromConnectors(): Promise<{ ingested: number; risks: DetectedRisk[] }> {
    if (this.hub) {
  ...
  async runBrief(userMessage: string): Promise<string> {
    return this.runMorningBrief(userMessage);
  }

  private async syncContextFromSources(): Promise<void> {
    if (this.hub) await this.hub.scanAll();
    await this.stateRepo.setLastScanAt(new Date().toISOString());
    if (this.contextGraph) {
      await this.contextGraph.syncFromSources();
    } else {
  ...
  private async formatBrief(
    ...
    else lines.push("- None above threshold");

    lines.push(
      "",
      "Prepared actions:",
      risks[0]?.preparedWork ?? "Say what to draft (reply, checklist, tweet fix).",
      "",
      morning ? "Recommended first action:" : "Suggested tomorrow:",
      risks[0]?.recommendedAction ?? "Pick highest-score risk above.",
      "",
      `Notify threshold: ${this.policy.immediate_score_min} | Brief min: ${this.policy.daily_brief_score_min}`,
    );
    ...
    if (this.cf && userMessage.trim()) {
      const advice = await this.cf.chat(userMessage, {
        ...
        system:
          "Chief of staff. Given brief data, suggest 3 concrete next steps with prepared work. Short iMessage style.",
      });
      lines.push("\n## CoS notes\n", advice);
    }
