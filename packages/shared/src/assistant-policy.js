...
export function withAssistantPolicy(system) {
    const parts = [system?.trim(), HERMES_ASSISTANT_POLICY].filter(Boolean);
    return parts.join("\n\n");
}
export function withMessagingPersona(system, channel) {
    const base = system?.trim();
    if (!isMessagingChannel(channel))
        return withAssistantPolicy(base);
    return withAssistantPolicy([base, HERMES_MESSAGING_PERSONA].filter(Boolean).join("\n\n"));
}
/** @deprecated use withMessagingPersona */
export function withChatPersona(system, channel) {
    return withMessagingPersona(system, channel);
}
/** Only when the user clearly wants X/Twitter — not generic "post" in other contexts. */
export function wantsExplicitTweet(text) {
    return /\b(tweet|post a tweet|post to x|post on twitter|compose (?:a )?tweet)\b/i.test(text);
}
export function wantsShoppingOrLinks(text) {
    return /\b(link|links|url|buy|purchase|order|amazon|price|where (?:to|can i) (?:buy|get)|best .{0,40}(?:for|to buy)|gimme|give me|send me|find me)\b/i.test(text);
}
/** Gmail, social, inbox — always browser + autonomous loop, never API-only chat. */
export function needsBrowserAutonomy(text) {
    return /\b(gmail|google mail|inbox|unread|email|twitter|linkedin|x\.com|feed|calendar|slack|notion|github|check my|analyze my|scan my|monitor|open\s+https?)\b/i.test(text);
}
export function needsAgentPlanner(text) {
    return (needsBrowserAutonomy(text) ||
        wantsShoppingOrLinks(text) ||
        /\b(and then|check|open|run|build|create|implement|write|fix|fetch|send|install|tool|macro|code|execute|task|do this|help me|continue|proceed|figure out|think)\b/i.test(text));
}
...
export function extractHttpsLinks(text, max = 8) {
...
export function formatLinksFallback(links, preamble) {
    if (!links.length) {
        return (preamble ??
...
//# sourceMappingURL=assistant-policy.js.map