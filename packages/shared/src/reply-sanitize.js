...
export function stripModelReasoning(text) {
    ...
    const formulateSection = trimmed.match(/\*\*Formulate(?: the response)?:?\*\*([\s\S]*)/i);
    if (formulateSection) {
    ...
    if (looksLikeLeakedReasoning(trimmed)) {
        ...
        for (let i = lines.length - 1; i >= 0; i--) {
            ...
            if (cleaned.length >= 8 && !looksLikeLeakedReasoning(cleaned))
                return cleaned;
        }
        return "";
    }
...
export const MESSAGING_RETRY_SYSTEM = "Your previous reply was rejected — it exposed internal reasoning steps. " +
...
export function messagingModelId() {
    return (process.env.HERMES_MESSAGING_MODEL?.trim() ||
